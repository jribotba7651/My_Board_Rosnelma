...
class AddPowerMeView extends StatelessWidget {
  const AddPowerMeView({super.key});

  @override
  Widget build(BuildContext context) {
    var controller = Get.find<PowerMeController>();

    return Scaffold(
      backgroundColor: AppColors.whiteColor,
      bottomNavigationBar: Padding(
        padding: const EdgeInsets.all(16.0),
        child: GetBuilder<PowerMeController>(builder: (cont) {
          return ElevatedButtonW(
              buttonText: "Add",
              isLoading: cont.postingData,
              onTap: () {
                cont.addNewPowerMe(context);
              });
        }),
      ),
      body: SafeArea(
